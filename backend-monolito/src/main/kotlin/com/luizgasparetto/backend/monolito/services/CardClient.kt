...
class CardClient(
    ...
    /** Detalhes da cobrança de cartão */
    fun getCharge(chargeId: String): JsonNode {
        val url = "${baseUrl()}/v1/charge/$chargeId" // ✅ detalhes da transação
    ...
    /** Cancela transação (quando possível) */
    fun cancel(chargeId: String): Boolean {
        val url = "${baseUrl()}/v1/charge/$chargeId/cancel" // ✅ endpoint de cancelamento
        val token = auth.getAccessToken()
        val headers = HttpHeaders().apply {
            contentType = MediaType.APPLICATION_JSON
            setBearerAuth(token)
        }
        val res = rt.exchange(url, HttpMethod.PUT, HttpEntity<Void>(headers), String::class.java)
